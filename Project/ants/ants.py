    ...
    def double(self):
        ...
        while cur:
            if not cur.ant:
                cur = cur.exit
                continue
            if (not cur.ant.is_doubled):
                cur.ant.damage *= 2
                cur.ant.is_doubled = True
            if cur.ant.is_container and cur.ant.ant_contained and (not cur.ant.ant_contained.is_doubled):
                cur.ant.ant_contained.damage *= 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not cur.is_hive:
            bee = random_bee(cur.bees)
            # print('DEBUG:', bee.place.name)
            if bee:
                if step >= self.lower_bound and step <= self.upper_bound:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            add_damage += self.damage
        Ant.reduce_health(self, amount)
        bee_list = place.bees[:]
        for bee in bee_list:
            bee.reduce_health(add_damage)
        # END Problem 5
