...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        var = signature.first
        formal_par = signature.rest
        body = expressions.rest
        env.define(var, do_lambda_form(Pair(formal_par, body), env))
        return var
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        lst_names.append(bind.first)
        lst_vals.append(scheme_eval(bind.rest.first, env))
        bindings = bindings.rest
    for name in lst_names[::-1]:
        names = Pair(name, names)
    for val in lst_vals[::-1]:
        vals = Pair(val, vals)
