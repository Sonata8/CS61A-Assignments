...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    p1 = goal
    last = strategy(goal, goal)
    while p1 >= 0:
        p2 = goal
        while p2 >= 0:
            _strategy = strategy(p1, p2)
            if _strategy != last:
                return False
            p2 -= 1
        p1 -= 1
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    avrg = make_averaged(roll_dice, times_called)
    res = (1, avrg(1, dice))
    for i in range(2, 11):
        sc = avrg(i, dice)
        if sc > res[1]:
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    sc_boar = boar_brawl(score, opponent_score)
    if sc_boar >= threshold:
        return 0
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    sc = sus_update(0, score, opponent_score)
